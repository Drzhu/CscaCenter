...
public class ProvinceController {
	...
	public ProvinceController() {
		this.logger = Logger.getLogger(ProvinceController.class);
	}
	...
	public void txt_submit(HttpServletRequest request, HttpServletResponse resp,
		...
		String file_path = this.base+"/files"+areaid+".txt";
	...
	public void txt_nosubmit(HttpServletRequest request, HttpServletResponse resp,
		...
		for (THunit i : zlpg_list) {
			content += i.getCity() + "\t" + i.getHname() + "\t" + i.getHlevel() + "\t" + i.getJudgeman() + "\t"
					+ i.getJmtel() + "\r\n";
		}
		TxtUtils.contentToTxt(file_path, content);
		File file = new File(file_path);
	...
	public void excel_download(HttpServletRequest request, HttpServletResponse resp,HttpSession session,
			@RequestParam("areaid") String areaid) {
		
		HashMap<String, String> map = new HashMap<String, String>();
		map.put("result", "result");
		DataExportUtil deu = new DataExportUtil();
		List<SubmitHospital> zvList = zlpgValueService.downloadExcel(areaid);
		Date date = new Date();
		SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMddHHmmss");
		String filename = "已填写的Excel-" + sdf.format(date);
		int length = 10000;
				try {
					File file = deu.downloadExcel(zvList, length, filename, request);
					deu.dataExcelExcel(file, filename, resp);
//					System.out.println("Hello World!");
				} catch (IOException e) {
					e.printStackTrace();
				}finally {
					logger.info(session.getAttribute("username")+"已下载填表Excel");
				}
	}
	...
	public void excel_NotFilled(HttpServletRequest request, HttpServletResponse resp,HttpSession session,
			@RequestParam("areaid") String areaid) {
		HashMap<String, String> map = new HashMap<String, String>();
		map.put("result", "result");
		DataExportUtil deu = new DataExportUtil();
		...
		int length = 10000;
		try {
			File file = deu.downloadExcel(list, length, filename, request);
			deu.dataExcelExcel(file, filename, resp);
		} catch (IOException e) {
			e.printStackTrace();
		}finally {
			logger.info(session.getAttribute("username")+"已下载未填表Excel");
		}
	}
