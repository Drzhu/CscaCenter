...
<body style="padding: 0;" >
		...
		<div class="pinggu-cont">
			...
			<ul class="nav nav-pills nav-stacked nav-stacked-custom" role="tablist" id="myTab">
			  <li role="presentation" class="active"><a href="#shuoming" role="tab" data-toggle="tab" onclick="onloadmokuai()">评估表填写说明</a></li>
			  <li id="yl" role="presentation"><a href="#jiegou" role="tab" data-toggle="tab" onclick="onloadmokuai()">医疗结构资源配置</a></li>
			  <li id="sb" role="presentation"><a href="#shebei" role="tab"  data-toggle="tab" onclick="onloadmokuai()">设备资源配置</a></li>
			  <li id="rl" role="presentation"><a href="#renli" role="tab" data-toggle="tab" onclick="onloadmokuai()">人力资源配置</a></li>
			  <li id="zl" role="presentation"><a href="#zhiliao" role="tab" data-toggle="tab" onclick="onloadmokuai()">治疗技术</a></li>
			  <li id="jc" role="presentation"><a href="#jiance" role="tab" data-toggle="tab" onclick="onloadmokuai()">监测随访技术</a></li>
			  <li id="jx" role="presentation"><a href="#jiaoxue" role="tab" data-toggle="tab" onclick="onloadmokuai()">教学科研</a></li>
			  <li id="cz" role="presentation"><a href="#cuzhong" role="tab" data-toggle="tab" onclick="onloadmokuai()">卒中中心质量管理指标</a></li>
			  <li id="zz" role="presentation"><a href="#zuzhi" role="tab" data-toggle="tab" onclick="onloadmokuai()">组织管理质量</a></li>
			</ul>
			<!-- Tab panes -->
			<div class="tab-content">
			  	...
			  	<div class="dati-box">
				  	...
				  	<fieldset>
				  		...
				  		<div id="SR_A_NIOR7D" class="dati-control">
				  			<h4>5. 有能提供 24小时×7 天服务的神经介入手术室。</h4>
				  			<div class="sel-option">
				  				<label>
				  	...
				  	<div>
				  	<input style="margin-left: 40%;width: 130px" class="btn btn-success"   value="保存并下一页" onclick="nextPage(this)"  readonly="readonly"/>
				  	</div>
			  	</div>
			  ...
			  <div role="tabpanel" class="tab-pane" id="zuzhi">
			  	<div class="dati-box">
			  		...
			  		<div>
			  		<input  style="margin-left: 40%;width: 70px" class="btn btn-success"  value="提交"  onclick="submit_pinggu(this)" readonly="readonly"/>
			  		</div>
			  	</div>
			  </div>
			</div>
...
function nextPage(obj) {
	...
	var res = [];
	var json = {};
   	$("#"+pa+" input[type='radio']").each(function(){
   	...
   	for (var a = 0; a < res.length; a++) {
   		var pcode_radio = document.getElementsByName(res[a]);
		if (pcode_radio[0].checked != true && pcode_radio[1].checked != true) {
			if(res[a]=='MF_C_VS' || res[a]=='TSR_C_CE'){
				$("html,body").animate({scrollTop:0}, 500);
			}else if(res[a]=='QA_A_CP_d' || res[a]=='QA_A_CP_a'){
				$("html,body").animate({scrollTop:'350px'}, 500);//SR_A_BED40
			}else if(res[a]=='OM_A_PCP_a' || res[a]=='SR_A_HOSP' || res[a]=='OM_A_PDCA'){
				$("html,body").animate({scrollTop:'1250px'}, 500);
			}else if(res[a]=='ER_B_DSA' || res[a]=='ER_B_MRI' || res[a]=='OM_A_DP'){
				$("html,body").animate({scrollTop:'1200px'}, 500);
			}else if(res[a]=='RT_C_SS' || res[a]=='RT_C_MJC'){
				$("html,body").animate({scrollTop:'4000px'}, 500);
			}else if(res[a]=='HR_A_NIP_f' || res[a]=='HR_A_CEA' || res[a]=='HR_A_NIP_d'){
				$("html,body").animate({scrollTop:'4700px'}, 500);
			}else if(res[a]=='SR_A_BED40'){
				$("html,body").animate({scrollTop:'1600px'}, 500);
			}
			
			var pcode_name = res[a].split("_").join("");
			$("#" + pcode_name).focus(); 
			$("#" + res[a]).addClass("intro");
			toastr.warning('当前模块的选项还未选完！');
		 	 return false;
		}
	}
   	for (var j = 0; j < res.length; j++) {
   		var pcode_radio = document.getElementsByName(res[j]);
		 for (var i = 0; i < pcode_radio.length; i++) {
			if (pcode_radio[i].checked == true) {
			 	 var pcodeValue = pcode_radio[i].value;
			 	if(a==res.length-1){
			 		 pcode_values+=res[j]+"="+pcodeValue;
			 	}else{
			 		 pcode_values+=res[j]+"="+pcodeValue+",";
			 	}
...
function submit_pinggu(obj) {
	var pcode_values ='';
	var res = [];
	var json = {};
	for ( var m = 0; m < modular.length; m++) {
		$("#"+modular[m]+" input[type='radio']").each(function(){
	  		 var pname =$(this).attr("name");
	  		  if(!json[pname]){
	  			   res.push(pname);
	  			   json[pname] = 1;
	   	...
	   	for (var a = 0; a < res.length; a++) {
	   		var pcode_radio = document.getElementsByName(res[a]);
			if (pcode_radio[0].checked != true && pcode_radio[1].checked != true) {
				$('#myTab a[href="#'+modular[m]+'"]').tab('show');
				if(res[a]=='MF_C_VS' || res[a]=='TSR_C_CE'){
					$("html,body").animate({scrollTop:0}, 500);
				}else if(res[a]=='QA_A_CP_d' || res[a]=='QA_A_CP_a'){
	...
	$(obj).addClass('disabled'); 
		...
		success: function(data){
			if(data.result=='success'){
				swal({ 
				    title: "您确定要提交评估表吗？", 
				    text: "提交后不可修改，如需修改内容请联系联盟数据协调员！",
				    type: "success", 
				    showCancelButton: true, 
				    closeOnConfirm: true, 
				    confirmButtonText: "是的，我要提交", 
				    confirmButtonColor: "#4a4aff" 
				}, function() { 
					$(obj).addClass('disabled'); 
					$.ajax({
						type: "get",
						...
						success: function(data){
							if(data.result!='success'){
								toastr.error('提交失败，请检查网络环境后刷新页面');
								return false;
							}else{
								toastr.success("提交成功");
								history.go(-1);
...
function checkInput(){
	...
	var json = {};
   	...
   	for (var a = 0; a < res.length; a++) {
		$("input:radio[name="+res[a]+"]").change(function (){ 
			$(this).parent().parent().parent().removeClass("intro");
		});
	}
}

function onloadmokuai() {
	$("html,body").animate({scrollTop:0}, 1);
}
</script>
</html>
