...
public class HospitalController {
	...
	public String hospitalInfoInsert(HttpServletRequest request,HttpSession session,HttpServletResponse response,HospitalEdit hospitalEdit,String cYear,String cMonth,String otherDep) throws ParseException{
		...
		if(i==0){
			...
			if(hospitalEdit.getDist()==null){
				hospitalEdit.setDist("");
			}
	...
	public String hospitalInfoUpdate(HttpServletRequest request,HttpSession session,HttpServletResponse response,HospitalEdit hospitalEdit,String cYear,String cMonth,String otherDep) throws ParseException{
		...
		if(hospitalEdit.getCity()!=null && hospitalEdit.getDist()==null){
			hospitalEdit.setDist("");
		}
		if(hospitalEdit.getHlevel()!=null && !"".equals(hospitalEdit.getHlevel()) && !"未分级".equals(hospitalEdit.getHlevel())){
			String[] str = hospitalEdit.getHlevel().split("级");
	...
	public String selectHospVague(HttpServletRequest request, HttpSession session) {
		String hname = request.getParameter("hname");
	 	String hospName = "hospName="+hname;
	    String result = "";
	    result =sendGet("http://123.56.178.191:7070/queryHospInfo/queryHosp.action", hospName).replace("/n", "");
		return result;
	}
	//checkHospExist
	@RequestMapping(value = "checkHospExist", method = RequestMethod.GET, produces = "text/plain;charset=UTF-8")
	@ResponseBody
	public String checkHospExist(HttpServletRequest request, HttpSession session) {
		String hname = request.getParameter("hospName");
	 	String hospName = "hospName="+hname;
	    String result = "";
	    result =sendGet("http://123.56.178.191:7070/queryHospInfo/checkHospExists.action", hospName).replace("/n", "");
		return result;
	}
	
	@RequestMapping(value = "checkHospRepeat", method = RequestMethod.GET, produces = "text/plain;charset=UTF-8")
	@ResponseBody
	public String checkHospRepeat(HttpServletRequest request, HttpSession session) {
		HashMap<String, Integer> map = new HashMap<String, Integer>();
		String hname = request.getParameter("hospName");
		String hunitid = session.getAttribute("hunitid").toString();
		HospitalEdit he = new HospitalEdit();
		he.sethName(hname);
		he.setHunitid(hunitid);
		System.out.println(hname);
		int i = hospInfoService.checkHospRepeat(he);
		map.put("repeat", i);
		return JsonUtil.map2json(map);
	}
